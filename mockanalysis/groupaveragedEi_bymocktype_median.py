...
if __name__=='__main__':
    ...
    for sd in subdirs:
        ...
        for fname in files:
            if i<30:
                ...
                mock = mock[(mock.g3logmh_l>0)] # incl. in group finding
                mock.loc[:,'g3logmh_l']=mock.g3logmh_l-np.log10(HUBBLE_CONST/100.)
                mock.loc[:,'loghalom']=mock.loghalom-np.log10(HUBBLE_CONST/100.)
                mock.loc[:,'Ei'] = (mock.g3logmh_l-mock.loghalom)
                mock = mock.groupby('g3grp_l').median()
                print(len(mock))
                print(mock.Ei)
                errors.append(mock.Ei) # Ei now means group-averaged Ei
    ...
    def plot_ref_shading(char):
        ...
        if char=='D':
            axd[char].set_ylabel(r"Group Median $\log M_{\rm HAM} - \log M_{\rm true}$")
    plot_ref_shading('B')
    ...
    axd['B'].annotate("G3 Groups (Fiducial)",xy=(11.2,1),fontsize=10)
    axd['B'].set_ylim(-3,1.5)
    ...
    axd['C'].annotate("G3 Groups (Central-Offset)",xy=(11.2,1.),fontsize=10)
    axd['C'].set_ylim(-3,1.5)
    ...
    axd['D'].set_ylim(-3,1.5)
    #axd['D'].set_title(r"G3 Group Finder ($b_v=0.8$)")
    axd['D'].annotate("G3 Groups "+r"($b_v=0.8$)",xy=(11.2,1.))
    ...
    axd['E'].set_ylim(-3,1.5)
    #axd['E'].set_title(r"G3 Group Finder ($b_v=1.2$)")
    axd['E'].annotate("G3 Groups "+r"($b_v=1.2$)",xy=(11.2,1.))
    ...
    for sd in subdirs:
        ...
        for fname in files:
            if i<30:
                ...
                mock.loc[:,'loghalom']=mock.loghalom-np.log10(HUBBLE_CONST/100.)
                mock.loc[:,'Ei'] = (mock.fofe17logmh-mock.loghalom)
                mock = mock.groupby('fofe17id').median()
                print(len(mock))
                print(mock.Ei)
                errors.append(mock.Ei) # Ei now means group-averaged Ei
    ...
    axd['A'].annotate("FoF Groups (Fiducial)",xy=(11.2,1.),fontsize=10)
    axd['A'].set_ylim(-3,1.5)
