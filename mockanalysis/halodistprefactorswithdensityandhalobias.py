...
if __name__=='__main__':
    ...
    for ii in range(0,2):
        for jj in range(0,2):
            ...
            for fname in files:
                ...
                print(currentsd,fname,len(mock))
                dens.append(len(mock)/192351./(0.7**3.))
                scale.append(mad(dm))

                mock = mock[mock.g3grpn_l>1]
                mock = mock.groupby('g3grp_l').first()
                comp.append(np.array(mock.halo_comp))
                purity.append(np.array(mock.halo_purity))
            ...
            axs[ii][jj].scatter(xpos, [np.mean(xx) for xx in purity], marker='*', s=60, label=r'Mean ${P}_h$', edgecolor='blue', facecolor='None', zorder=99)
            axs[ii][jj].scatter(xpos, [np.median(xx) for xx in purity], color='blue', marker='+', s=40, label=r'Median ${P}_h$',zorder=98)
            ...
            for pc in parts['bodies']:
                ...
                pc.set_alpha(0.7)
            axs[ii][jj].scatter(xpos, [np.mean(xx) for xx in comp], marker='.', s=40, label=r'Mean ${C}_h$',color='darkorange')
            axs[ii][jj].scatter(xpos, [np.median(xx) for xx in comp], marker='s', s=40, label=r'Median ${C}_h$', edgecolor='darkorange', facecolor="None")
    ...
    plt.tight_layout()
    plt.savefig("../figures/prefacs_with_dens_halo.pdf",dpi=300)
    plt.show()
