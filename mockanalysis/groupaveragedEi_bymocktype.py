...
if __name__=='__main__':
    ...
    for sd in subdirs:
        ...
        for fname in files:
            if i<30:
                ...
                mock = mock.groupby('g3grp_l').mean()
                print(len(mock))
                print(mock.Ei)
    ...
    def plot_ref_shading(char):
        ...
        if char=='D':
            axd[char].set_ylabel(r"Group-averaged $\log M_{\rm HAM} - \log M_{\rm true}$")
    plot_ref_shading('B')
    ...
    axd['B'].annotate("G3 Groups\n(Fiducial)",xy=(11.2,1.7),fontsize=10)
    axd['B'].set_ylim(-2.5,2.5)
    ...
    axd['C'].annotate("G3 Groups\n(Central-Offset)",xy=(11.2,1.7),fontsize=10)
    axd['C'].set_ylim(-2.5,2.5)
    ...
    for sd in subdirs:
        ...
        for fname in files:
            if i<30:
                ...
                mock = mock.groupby('fofe17id').mean()
                print(len(mock))
                print(mock.Ei)
                errors.append(mock.Ei) # Ei now means group-averaged Ei
