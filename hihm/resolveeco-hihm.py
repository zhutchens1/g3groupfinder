...
if __name__=='__main__':
    ...
    fig, (ax1,ax3) = plt.subplots(figsize=doublecolsize, nrows=1, ncols=2)
    ax1.annotate("RESOLVE-B + ECO:\n G3 Groups", xy=(12.3,7.4), )
    ...
    ax1.errorbar(bincenters, median_central_hi, yerr=median_central_hi_error, fmt='^-', color='green', label=r'$M_{\rm HI,cen}$', rasterized=True)
    ...
    ax1.scatter(sats.g3logmh_l, sats.totalhi_sat, marker='.', color='mediumorchid', alpha=0.7, s=5, label='Satellite-Integrated HI', rasterized=True)
    ...
    median_logmh_error = np.std(np.array([sbs(ecog3groupslogmh[np.where(np.logical_and(ecog3groupslogmh>binedges[i-1], ecog3groupslogmh<=binedges[i]))],\
                               5000, np.median, kwargs=dict({'axis':1})) for i in range(1,len(binedges))]), axis=1)
    ax1.errorbar(bincenters, median_totalhi, yerr=median_totalhi_error, xerr=median_logmh_error, fmt='^-', label=r'$M_{\rm HI,\, grp}$', color='k', rasterized=True)
    #############################################
    ...
    ax3.annotate("RESOLVE-B + ECO:\n FoF Groups", xy=(12.3,7.4), )
    ...
    poptfof, pcovfof = np.polyfit(bincenters[bincenters<=11.5], median_totalhi[bincenters<=11.5], deg=1, cov=True)
    print("best fit for fof (<11.5): ", poptfof, np.sqrt(np.diag(pcovfof)))
    poptfof_2, pcovfof_2 = np.polyfit(bincenters[bincenters>11.5], median_totalhi[bincenters>11.5], deg=1, cov=True)
    print("best fit for fof_2 (>11.5): ", poptfof_2, np.sqrt(np.diag(pcovfof_2)))
    
    sellow = ecofofgroupslogmh<11.5
    residlow = np.abs(ecofofgroupslogmgas[sellow]-np.poly1d(poptfof)(ecofofgroupslogmh[sellow]))
    selhigh = ~sellow
    residhigh = np.abs(ecofofgroupslogmgas[selhigh]-np.poly1d(poptfof_2)(ecofofgroupslogmh[selhigh]))
    resid = np.concatenate([residlow,residhigh])

    print("median scatter: ", np.median(resid))
    #############################################
    ######################################
    ax3.set_xlabel(r"log group $M_{\rm vir}$",) 
    #ax3.legend(loc='best')
    ax3.set_ylim(7,12)
    ax3.axvline(11.4, color='k', alpha=0.1)
    ax3.axvline(12.1, color='k', alpha=0.1)
    plt.tight_layout()
    plt.savefig("ecoresolve_HIHMrelation.pdf")
    plt.show()

    foftable = dict({r'$logM_h$':bincenters, 'median group HI':median_totalhi, 'median group HI error':median_totalhi_error,
                     'median satellite-summed HI':median_sat_totalhi, 'median satellite-summed HI error':median_sat_hi_error,
                    'median central HI':median_central_hi, 'median central HI error':median_central_hi_error})
    foftable = pd.DataFrame(foftable)


    ###############################################
    ###############################################
    # Produce Tables
    print(g3table), print(foftable)
    if True:
        print(g3table.to_latex(index=False, float_format='%0.2f'))
        print(foftable.to_latex(index=False, float_format='%0.2f'))
